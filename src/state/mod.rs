...
use crate::{
  config::Config,
  io::Event,
  state::handler::{Active, DEFAULT_VIEW, View, ViewId},
};
use rspotify::model::{
  CurrentPlaybackContext, Page, PlayableItem, PlaylistItem, SimplifiedPlaylist,
};
...
impl State {
  ...
  pub fn push_view(&mut self, next_id: ViewId, next_active: Active) {
    if !self
      .navigation
      .last()
      .map(|last| last.id == next_id)
      .unwrap_or(false)
    {
